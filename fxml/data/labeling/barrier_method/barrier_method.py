...
class BarrierMethod:
    ...
    def __init__(
        ...
        self.conditions = BarrierConditions(
            n=self.n_barriers, barrier=barrier
        ).conditions
        # Caching
    ...
    def _get_cumulative_returns(self) -> pd.DataFrame:
        ...
        for i in range(1, self.n + 1):
            cumulative_returns[i] = (
                np.exp(np.log(self.returns + 1).rolling(i).sum().shift(-i)) - 1
            )

            if self.center:
                cumulative_returns[i] = (
                    cumulative_returns[i] - cumulative_returns[i].expanding().mean()
                )
    ...
    def time_since_last_crossing(self) -> pd.DataFrame:
        ...
        for label in barrier_crossings.columns:
            time_since_last_crossing[label] = (
                crossing_seen[label].groupby(crossing_groups[label]).cumsum()
            )
    ...
    def transition_probabilities(self) -> pd.DataFrame:
        ...
        if self._labels is None:
            raise AttributeError(
                "The attribute 'labels' have not been calculated yet. Run .labels"
            )
        transition_counts = self.labels.groupby(
            [self.labels.rename("From"), self.labels.shift(-1).rename("To")]
        ).size()

        transition_probas = transition_counts.groupby(level=0).apply(
            lambda x: x / x.sum()
        )
    ...
    def signals_pa(self, factor: int = 252) -> float:
        ...
        """
        return (
            (self.labels != self.labels.shift()).dropna().sum()
            / self.labels.dropna().shape[0]
            * factor
        )

    def plot_at_date(self, date: str, offset=30, figsize=(12, 3)) -> None:
        """
        ...
        offset (int): Number of bars to consider before and after the given date for the plot. Default is 30 bars.
        ...
        if date not in index.index:
            warnings.warn_explicit(
                f"The selected date '{date.strftime('%Y-%m-%d %H:%M:%S')}' is not inside the attribute 'returns'."
                + f"The next available date is '{index[date:].index.min().strftime('%Y-%m-%d %H:%M:%S')}'.",
                UserWarning,
                "",
                1,
            )
        ...
        for key in self.conditions.keys():
            barriers.append(
                pd.Series(100 * (1 + key * self.barrier), index=barrier_idx).rename(key)
            )
        ...
        aut_locator = mdates.AutoDateLocator()
        ...
        for col in barriers.columns:
            ...
            ax.text(
                x=text_date,
                y=barriers[col].min() - text_spacing_y,
                s=col,
                style="normal",
                c=c,
            )
        ...
        plt.plot(
            [barriers.index.min(), barriers.index.min()],
            [barriers[-self.n_barriers].min(), barriers[self.n_barriers].max()],
            c="grey",
            ls="dotted",
        )
        plt.plot(
            [barriers.index.max(), barriers.index.max()],
            [barriers[-self.n_barriers].min(), barriers[self.n_barriers].max()],
            c="black",
        )
